...
export async function handleV19MigrationApi(request, env, session) {
  ...
  if (url.pathname === '/api/migration/sqlite/status' && request.method === 'GET') {
    return handleStatus(env.DB);
  }
  if (url.pathname === '/api/migration/sqlite/preview' && request.method === 'POST') {
    return handlePreview(request, env.DB);
  }
  if (url.pathname === '/api/migration/sqlite/start' && request.method === 'POST') {
    return handleStart(request, env.DB, session);
  }
  if (url.pathname === '/api/migration/sqlite/chunk' && request.method === 'POST') {
    return handleChunk(request, env.DB);
  }
  if (url.pathname === '/api/migration/sqlite/finish' && request.method === 'POST') {
    return handleFinish(request, env.DB);
  }
  if (url.pathname === '/api/migration/sqlite/abort' && request.method === 'POST') {
    return handleAbort(request, env.DB);
  }
...
async function handleStatus(db) {
  const active = await loadActiveRun(db);
  const latest = active || await db.prepare(`
    SELECT *
    FROM legacy_migration_runs
    ORDER BY created_at DESC
    LIMIT 1
...
async function handlePreview(request, db) {
  ...
  if (!parsed.ok) return json({ ok: false, error: parsed.error, code: parsed.code || 'MIGRATION_SOURCE_INVALID' }, 400);

  const active = await loadActiveRun(db);
  if (active) {
    const same = String(active.dataset_sha256) === parsed.manifest.datasetSha256;
    return json({
      ok: true,
      canStart: false,
      canResume: same,
      activeRun: await runView(db, active),
      target: await targetState(db),
...
async function handleStart(request, db, session) {
  ...
  const active = await loadActiveRun(db);
  if (active) {
    if (String(active.dataset_sha256) !== parsed.manifest.datasetSha256) {
      return json({ ok: false, error: '已有其他舊帳本遷移作業進行中。', code: 'MIGRATION_ALREADY_ACTIVE' }, 409);
    }
    return json({ ok: true, resumed: true, run: await runView(db, active) });
  }
  ...
  const chunkRows = [];
  for (const chunk of manifest.transactionChunks) {
    chunkRows.push([runId, 'transactions', chunk.index, chunk.rowCount, chunk.sha256, 'pending']);
  }
  for (const chunk of manifest.openingBalanceChunks) {
    chunkRows.push([runId, 'opening_balances', chunk.index, chunk.rowCount, chunk.sha256, 'pending']);
  }
...
async function handleChunk(request, db) {
  ...
  if (!/^mig_[0-9a-f-]{36}$/i.test(runId) || !['transactions', 'opening_balances'].includes(kind)
      || !Number.isInteger(index) || index < 0 || !rows || rows.length > MAX_CHUNK_ROWS
  ...
  if (String(expected.status) === 'imported') {
    if (String(expected.expected_sha256) !== suppliedSha || Number(expected.expected_rows) !== rows.length) {
  ...
  if (Number(expected.expected_rows) !== rows.length || String(expected.expected_sha256) !== suppliedSha) {
    return json({ ok: false, error: '遷移分段筆數或摘要與預覽不一致。', code: 'MIGRATION_CHUNK_MISMATCH' }, 409);
  }

  const normalized = kind === 'transactions'
    ? validateTransactionRows(rows)
    : validateOpeningBalanceRows(rows);
  if (!normalized.ok) return json({ ok: false, error: normalized.error, code: 'MIGRATION_CHUNK_DATA_INVALID' }, 400);
  ...
  const references = await referenceSets(db);
  const referenceError = validateReferences(kind, normalized.rows, references);
  if (referenceError) return json({ ok: false, error: referenceError, code: 'MIGRATION_REFERENCE_INVALID' }, 400);

  const now = new Date().toISOString();
  const statements = [];
  if (kind === 'transactions') {
    statements.push(...multiInsert(db, 'transactions',
    ...
    statements.push(...multiInsert(db, 'opening_balances',
...
async function handleFinish(request, db) {
  ...
  if (mismatch) {
    await db.prepare(`
      UPDATE legacy_migration_runs SET status = 'failed', last_error = ?, updated_at = ? WHERE run_id = ?
    `).bind(mismatch, new Date().toISOString(), runId).run();
    return json({ ok: false, error: `遷移後核對失敗：${mismatch}`, code: 'MIGRATION_RECONCILIATION_FAILED' }, 409);
  }
...
async function handleAbort(request, db) {
  const body = await readJson(request);
  const runId = String(body?.runId || '');
  if (!/^mig_[0-9a-f-]{36}$/i.test(runId) || body?.confirm !== true || String(body?.confirmation || '') !== '中止遷移') {
  ...
  try {
    await db.batch([
      ...
      db.prepare(`
        ...
        SELECT 'income', id, '一般收入', 0, 0, ? FROM category_groups WHERE kind = 'income' AND name = '收入分類'
      `).bind(now),
      db.prepare(`
        INSERT INTO categories(kind, group_id, name, sort_order, is_favorite, created_at)
        SELECT 'expense', id, '一般支出', 0, 0, ? FROM category_groups WHERE kind = 'expense' AND name = '支出分類'
      `).bind(now),
      db.prepare(`
        UPDATE legacy_migration_runs
...
async function validateMigrationStartPayload(body) {
  const manifest = normalizeManifest(body?.manifest);
  if (!manifest.ok) return manifest;
  const structure = normalizeStructure(body?.structure, manifest.value.sourceSchemaVersion);
  if (!structure.ok) return structure;

  const m = manifest.value;
  const s = structure.value;
  if (m.counts.accounts !== s.accounts.length || m.counts.categoryGroups !== s.categoryGroups.length
      || m.counts.categories !== s.categories.length) {
    return fail('主檔筆數與遷移清單不一致。');
  }

  const refError = validateStructureReferences(s);
  if (refError) return fail(refError);

  const structureSha = await sha256Hex(JSON.stringify(s));
  if (structureSha !== m.structureSha256) return fail('主檔完整性摘要不一致。', 'MIGRATION_STRUCTURE_DIGEST_MISMATCH');

  return { ok: true, manifest: m, structure: s };
}
...
function normalizeManifest(value) {
  ...
  if (!sourceFileName || !Number.isInteger(sourceFileSize) || sourceFileSize <= 0 || sourceFileSize > MAX_SOURCE_FILE_BYTES) return fail('來源檔案大小不支援。');
...
function normalizeChunkManifest(value, expectedTotal) {
  ...
  if (!rows.length && expectedTotal !== 0) return fail('遷移分段清單不完整。');
  if (rows.length > MAX_CHUNKS) return fail('遷移分段數量過多。');
  let total = 0;
  const seen = new Set();
  const normalized = [];
  for (const item of rows) {
    ...
    if (!Number.isInteger(index) || index < 0 || seen.has(index) || !Number.isInteger(rowCount)
        || rowCount < 1 || rowCount > MAX_CHUNK_ROWS || !isSha256(sha256)) return fail('遷移分段清單格式錯誤。');
    seen.add(index);
    total += rowCount;
...
function normalizeAccounts(rows) {
  if (!Array.isArray(rows) || !rows.length || rows.length > MAX_ACCOUNTS) return fail('帳戶主檔資料量不正確。');
  const ids = new Set();
  const names = new Set();
  let defaults = 0;
  const out = [];
  for (const item of rows) {
    ...
    if (!id || !name || sortOrder === null || ![0, 1].includes(isDefault) || !createdAt || ids.has(id) || names.has(name)) return fail('帳戶主檔內容不正確。');
    ids.add(id); names.add(name); defaults += isDefault;
    out.push({ id, name, sortOrder, isDefault, createdAt });
  }
...
function normalizeGroups(rows) {
  if (!Array.isArray(rows) || !rows.length || rows.length > MAX_GROUPS) return fail('大分類主檔資料量不正確。');
  const ids = new Set();
  const keys = new Set();
  const out = [];
  for (const item of rows) {
    ...
    const createdAt = cleanTimestamp(item?.createdAt);
    const key = `${kind}\u0000${name}`;
    if (!id || !['income', 'expense'].includes(kind) || !name || sortOrder === null || !createdAt || ids.has(id) || keys.has(key)) return fail('大分類主檔內容不正確。');
    ids.add(id); keys.add(key);
    out.push({ id, kind, name, sortOrder, createdAt });
  }
  return { ok: true, rows: out };
}

function normalizeCategories(rows, sourceSchemaVersion) {
  if (!Array.isArray(rows) || !rows.length || rows.length > MAX_CATEGORIES) return fail('科目主檔資料量不正確。');
  const ids = new Set();
  const keys = new Set();
  const out = [];
  for (const item of rows) {
    ...
    if (!id || !groupId || !['income', 'expense'].includes(kind) || !name || sortOrder === null
        || ![0, 1].includes(isFavorite) || !createdAt || ids.has(id) || keys.has(key)) return fail('科目主檔內容不正確。');
    ids.add(id); keys.add(key);
    out.push({ id, kind, groupId, name, sortOrder, isFavorite, createdAt });
  }
  return { ok: true, rows: out };
}

function normalizeSettings(rows) {
  if (!Array.isArray(rows)) return fail('帳本設定資料格式錯誤。');
  const seen = new Set();
  const out = [];
  for (const item of rows) {
    ...
    if (key === 'frequent_summary_recent_count' && (!/^\d+$/.test(value) || Number(value) < 10 || Number(value) > 1000)) return fail('常用摘要最近筆數設定錯誤。');
    if (key === 'frequent_summary_min_count' && (!/^\d+$/.test(value) || Number(value) < 2 || Number(value) > 20)) return fail('常用摘要最低次數設定錯誤。');
    seen.add(key);
    out.push({ key, value });
  }
...
function validateTransactionRows(rows) {
  ...
  for (const item of rows) {
    ...
    const kind = String(item?.kind || '');
    const categoryName = cleanName(item?.categoryName);
    ...
    if (!id || ids.has(id) || !isDate(txDate) || !accountName || !['income', 'expense'].includes(kind)
        || !categoryName || summary.length > MAX_SUMMARY_LENGTH || !Number.isInteger(amount) || amount < 1 || amount > 9_999_999
        || !createdAt || !updatedAt) return fail('交易資料內容不正確。');
    ids.add(id);
    out.push({ id, txDate, accountName, kind, categoryName, summary, amount, createdAt, updatedAt });
  }
  return { ok: true, rows: out };
}

function validateOpeningBalanceRows(rows) {
  const keys = new Set();
  const out = [];
  for (const item of rows) {
    ...
    if (!isMonth(month) || !accountName || !Number.isSafeInteger(amount) || !createdAt || !updatedAt || keys.has(key)) return fail('期初餘額資料內容不正確。');
    keys.add(key);
    out.push({ month, accountName, amount, createdAt, updatedAt });
  }
  return { ok: true, rows: out };
}
...
function validateReferences(kind, rows, references) {
  if (kind === 'transactions') {
    for (const row of rows) {
      if (!references.accounts.has(row.accountName)) return `交易帳戶「${row.accountName}」不存在於來源主檔。`;
      if (!references.categories.has(`${row.kind}\u0000${row.categoryName}`)) return `交易科目「${row.categoryName}」不存在於來源主檔。`;
    }
    return '';
  }
  for (const row of rows) {
    if (!references.accounts.has(row.accountName)) return `期初餘額帳戶「${row.accountName}」不存在於來源主檔。`;
  }
  return '';
}
...
async function referenceSets(db) {
  ...
  return {
    accounts: new Set((accounts.results || []).map(row => String(row.name))),
    categories: new Set((categories.results || []).map(row => `${row.kind}\u0000${row.name}`))
  };
}

async function targetState(db) {
  const active = await loadActiveRun(db);
  if (active) return { ready: false, reason: 'migration_active', activeRunId: String(active.run_id) };
  ...
  const seedOnly = a.length === 1 && a[0].name === '現金' && Number(a[0].is_default) === 1
    && g.length === 2 && g.some(row => row.kind === 'income' && row.name === '收入分類') && g.some(row => row.kind === 'expense' && row.name === '支出分類')
    && c.length === 2 && c.some(row => row.kind === 'income' && row.name === '一般收入' && row.group_name === '收入分類')
...
async function loadActiveRun(db) {
  return db.prepare(`
...
async function runView(db, run) {
  const progress = await db.prepare(`
    ...
    GROUP BY chunk_kind
  `).bind(run.run_id).all();
  const chunks = { transactions: { total: 0, imported: 0 }, openingBalances: { total: 0, imported: 0 } };
...
function multiInsert(db, table, columns, rows) {
  ...
  const allowed = new Set(['accounts', 'category_groups', 'categories', 'transactions', 'opening_balances', 'legacy_migration_chunks']);

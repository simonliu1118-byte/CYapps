# CYSmartERP Project Rules

本文件只記錄 `apps/CYSmartERP/**` 的專案專屬永久規則；共通規則與 Public repository 政策分別以根目錄 `REPOSITORY_RULES.md`、`REPO_POLICY.md` 為準，不在此重複。

## 1. 專案定位

- 正式名稱：`CYSmartERP`；中文顯示名稱為「SMART ERP 自動打單工具」。
...
- 任一自動流程必須支援 `Esc` 緊急停止；停止只中止 CYSmartERP 後續自動行為，不得替使用者按 ERP「取消」。

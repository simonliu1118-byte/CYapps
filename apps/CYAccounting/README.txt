...
- 同步保留正式 ACC.svg 與 256px app.png；Windows CI 會驗證 ICO 七層尺寸與最終 EXE associated icon resource。
...
- 交易、期初、帳戶、科目、鎖帳、本機偏好及 Google Drive 本機連結全部重置；清除前先留可還原的本機備份，既有本機與雲端備份也保留。自訂資料庫位置仍指向當前位置。DELETE 是防誤觸確認文字，不是管理密碼。
- Actions 測試包解壓縮一次即為 CYAccounting 資料夾，不會有第二層 ZIP。

V1.0.27 更新重點
- Windows 深色模式時保持淺色標題列和清單；修正帳戶與科目清單看不清楚的問題。
- Build 1 曾移除清除全部功能及固定密碼；Build 2 依使用者決定恢復按鈕，仍不採固定密碼。
- SHA-256 於建置時檢查。
- 預設帳本在可攜程式資料夾的 Data；僅刪除 EXE 不會清除資料，改放其他位置的資料和 Google Drive 備份也不會隨程式資料夾刪除。

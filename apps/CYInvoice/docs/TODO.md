...
5. 人工待辦類型使用「折讓人工處理」，沿用既有 `InvoiceSyncIssue`／`ResolvedUtc` 資料模型；保存必要的發票識別、原申請員工編號、折讓原因與提出時間，不保存員工密碼。
6. 同一張發票已有未解決的「折讓人工處理」時不得重複新增第二筆；應沿用現有 unresolved issue。
7. 管理員至光貿網站人工完成實際折讓後，沿用「上傳問題」既有**標記已解決**資料路徑結案，不另建立「已處理」狀態或折讓專用結案按鈕。
8. `invoice_query` 的折讓資訊解析納入既有共用 QueryResult／官方資料更新流程；任何既有流程執行 `invoice_query` 時都使用同一套解析與收斂邏輯。
9. 若 `invoice_list` 無法提供足夠折讓資訊，應擴充既有「官方清單不足時需要 authoritative query」的共用 reconciliation 機制；啟動、5 分鐘、手動與每日同步仍共用既有同步服務，不建立折讓專用同步排程，也不只綁定每日同步。
10. 正常 query 已確認光貿存在折讓資料時，可自動 resolve 對應仍未解決的「折讓人工處理」；若管理員已手動標記已解決，後續 query 不因暫時查不到而自動重開該筆人工待辦。

實作前需注意：

- 目前一般「標記已解決」沒有管理員驗證；若折讓人工處理要求只有管理員可結案，應優先把權限檢查放進**共用人工結案入口**，不要做折讓專用 resolve 流程。
...
- 折讓不得改寫既有發票主狀態為「已折讓」；發票仍依官方規則維持已開立／已作廢等主狀態，折讓資訊應作為獨立官方資訊處理。
- 已作廢、等待作廢、開立失敗或官方狀態不明的發票，不應建立新的折讓人工處理；與既有作廢流程衝突時優先停止，不建立互相矛盾的待辦。

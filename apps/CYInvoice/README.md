...
- V2.4.0 尚未建立正式 Release；只有使用者於當次工作明確要求 `release` 時，才可從 `main` 啟動正式 Release workflow。
...
- 正式環境以 `/json/invoice_list` 執行最近 3 天同步；程式啟動、每 5 分鐘背景同步及手動「重新整理狀態」共用同一套同步核心，手動重新整理有 30 秒冷卻，重疊同步直接略過、不排隊。
...
- 已開立發票頁新增「同步問題」視窗，可查看目前帳號未解決問題，並手動標記完成、刪除或重新整理。若同一發票號碼或 OrderID 對到多筆本機紀錄，同步會停止自動猜測並留下問題紀錄。
...
- 舊 `Data/invoices.json`、`Data/buyer_names.json` 只作為首次 SQLite 遷移來源／保留備份，不再是 V2.4.0 的主要寫入資料庫。
...
正式 Release 只有在使用者當次工作明確要求 `release` 後才執行；不得因版本號已推進到 V2.4.0 而自動發布。

...
目前 Worker Email transport 已保留以下 adapter：

- `brevo`：目前 reference backend 預設開發 Provider。
- `resend`：未來取得自有網域後可直接切回。

切換 Provider 只修改 Worker runtime configuration，不修改 Windows Client，也不改 OTP API contract。

## Brevo runtime configuration
...
3. 將 `EMAIL_PROVIDER` 切換為 `resend`。
4. 將 Resend API Key 設為 `RESEND_API_KEY` Cloudflare Worker Secret，並更新 `EMAIL_FROM`。

...
V2.5 員工帳號／權限／發票作廢，以及 V2.6 暫行人工折讓、pending 追蹤、管理員結案、V2.6.2 折讓 PDF／折讓作廢人工待辦，均已完成工程實作並通過 Windows engineering CI。以下仍需真實 Windows／光貿環境驗證：
...
- [ ] 超過兩期的作廢／折讓 pending 顯示「可結案」，只有 ADMIN／SUPER_ADMIN 可結案；結案後停止本機追蹤並恢復 retention 清理資格。
...
- [ ] `AllowancePdfService`：簽章內容、code 15 校時重試、三種 style、可信任 `invoice.amego.tw` URL、非 PDF 拒絕、Cache 命中與損壞 Cache 重抓。

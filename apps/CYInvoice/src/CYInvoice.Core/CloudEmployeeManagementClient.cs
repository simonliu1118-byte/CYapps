...
public sealed record CloudEmployeeCreateProposal(
    string EmployeeNo,
    string Name,
    string Email,
    string Role,
...
public sealed record CloudEmployeeCreateChallenge(
    string ChallengeId,
    string MaskedEmail,
    DateTimeOffset ExpiresAt,
    DateTimeOffset ResendAfter);

public sealed class CloudEmployeeManagementClient
{
    ...
    public CloudEmployeeManagementClient(HttpClient httpClient, Uri baseUri, string deviceToken)
    {
    ...
    public async Task<CloudEmployeeCreateChallenge> StartCreateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var document = await SendAsync(
            "v1/employees/create/challenge",
            Payload(actorEmployeeNo, actorPassword, proposal),
            cancellationToken);
        if (!document.RootElement.TryGetProperty("challenge", out var challenge) || challenge.ValueKind != JsonValueKind.Object)
            throw new InvalidDataException("Cloud Employee creation challenge is missing.");
        return new CloudEmployeeCreateChallenge(
    ...
    public async Task<CloudEmployeeTransitionIdentity> ConfirmCreateAsync(
        string actorEmployeeNo,
        string actorPassword,
        CloudEmployeeCreateProposal proposal,
        string challengeId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var payload = new
        {
            ...
            actorPassword,
            employeeNo = proposal.EmployeeNo.Trim(),
            name = proposal.Name.Trim(),
            email = proposal.Email.Trim(),
            role = proposal.Role.Trim(),
            credentialVerifier = proposal.CredentialVerifier.Trim(),
            challengeId = challengeId.Trim(),
            otp,
        };
        using var document = await SendAsync("v1/employees/create/confirm", payload, cancellationToken);
        if (!document.RootElement.TryGetProperty("employee", out var employee) || employee.ValueKind != JsonValueKind.Object)
            throw new InvalidDataException("Created Cloud Employee is missing from the response.");
        return ReadIdentity(employee);
    }

    private static object Payload(string actorEmployeeNo, string actorPassword, CloudEmployeeCreateProposal proposal) => new
    {
        actorEmployeeNo = actorEmployeeNo.Trim(),
        actorPassword,
        employeeNo = proposal.EmployeeNo.Trim(),
    ...
    private static void ValidateProposal(CloudEmployeeCreateProposal proposal)
    {
        ArgumentNullException.ThrowIfNull(proposal);
        var employeeNo = proposal.EmployeeNo.Trim();
        if (employeeNo.Length != 4 || !employeeNo.All(char.IsAsciiDigit))

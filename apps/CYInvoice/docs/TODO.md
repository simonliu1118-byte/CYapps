...
- [ ] A 機另提供「寄送新裝置邀請」：超管驗證後，將 API 網址與 72 小時、限用一次的邀請碼寄至超管帳號已驗證的 Email；邀請可由 A 機撤銷或重新寄送，重寄成功會撤銷同一 A 機先前尚未使用的邀請。B 機輸入網址、邀請碼、超管編號與密碼，確認 Workspace 名稱後加入；不再要求第二封 Email 驗證碼。A 機邀請視窗可顯示加入結果。

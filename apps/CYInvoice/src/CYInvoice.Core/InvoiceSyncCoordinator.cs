...
public sealed class InvoiceSyncCoordinator
{
    ...
    public Task<InvoiceSyncRunResult> RunStartupAsync(CancellationToken cancellationToken = default) =>
        RunAsync(manual: false, cancellationToken);
    ...
    public Task<InvoiceSyncRunResult> RunManualAsync(CancellationToken cancellationToken = default) =>
        RunAsync(manual: true, cancellationToken);

    private async Task<InvoiceSyncRunResult> RunAsync(bool manual, CancellationToken cancellationToken)
    {
        if (manual)
        {
        ...
        try
        {
            ...
            var result = await service.SyncRecentAsync(cancellationToken).ConfigureAwait(false);
            if (manual)
            {

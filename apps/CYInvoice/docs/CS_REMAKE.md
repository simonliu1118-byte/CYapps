...
`V1.1.0-cs.4` 修正 `cs.3` 啟動時，已開立清單在建立空白斑馬列期間同步清除目前儲存格所造成的 WinForms 重入風險。空白列仍不可操作，但改為只在視窗已建立且使用者實際移入後，以訊息佇列延後清除選取。程式入口新增啟動錯誤提示與本機 `Logs/startup-error.log`，Windows CI 也新增 WinForms 主視窗建構 smoke test，避免再交付雙擊後無畫面的版本。

...
> Current formal baseline: **V0.20.0**（2026-09-27）
...
- 桌面操作維持高資訊密度與鍵盤效率；Tablet / Mobile 使用同一網站的 RWD + Adaptive UI，不另拆第二套前端。
...
- **V0.20.0 RWD / Adaptive UI Phase 1**：Desktop 維持高密度帳務操作；Tablet 重排工具列與輸入欄位；Mobile 採單欄新增記帳、交易卡片、全螢幕設定／Modal、底部輸入確認 sheet 與較大觸控區；
...
已完成 Desktop UI/UX Phase 1～3，V0.20.0 開始正式進入 RWD / Adaptive UI。

目前 presentation 分層：

- Desktop `>= 1024px`：維持高資訊密度、鍵盤高效率輸入與完整帳務表格；
- Tablet `768–1023px`：相同功能，改以較少欄數與重新排列的工具列降低擁擠；
- Mobile `< 768px`：新增記帳改單欄觸控表單，交易由寬表格改成卡片，設定／Modal 改全螢幕 sheet，輸入確認改底部 sheet；
- 同一套資料與 API，不建立獨立手機網站；
- V0.20.0 為 Phase 1，仍需真實手機／平板畫面驗收後再進一步調整細節，不宣稱所有行動裝置已完成實機驗證。

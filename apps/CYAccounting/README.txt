...
Windows 可攜版
- 正式使用請至 GitHub Release 下載 `CYAccounting_V1.0.26_Windows.zip`。
- 解壓縮後開啟 `CYAccounting` 資料夾，直接執行 `CYAccounting.exe`；不需另行安裝 Python。
- 更新舊版本前先保留原本的 `Data` 資料夾，避免遺漏帳本、設定及備份。
...
- Windows CI 會建立乾淨可攜版並在全新暫存資料夾實際啟動；正式 Release 同時提供 ZIP 與 SHA-256。
